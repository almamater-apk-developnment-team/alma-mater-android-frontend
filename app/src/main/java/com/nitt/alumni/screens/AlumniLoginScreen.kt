...
@Composable
fun AlumniLoginScreen(navController: NavController){
        ...
        Button(
            onClick = {
                if (emailId.isNotEmpty() && passWord.isNotEmpty()) {
                    if(emailId == "almamateralumnitestaccount@almamater.com" && passWord == "Ghexsdrcftvyghujk3456789rtvgybhu56789cfvgbhnrftgy345!#%^FGBcxdrcftvgybhunjmk][]/‘ftgv#)+_)^^")
                    {
                        saveUserDetails(context = context,name = "Alumni Test Account", email = theUser.email, role = "alumni")
